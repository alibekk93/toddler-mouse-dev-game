...
- `categories` is a list, normalised exactly like `tags`. A picture belongs to as many as fit it — a yellow square is a colour *and* a shape — because forcing one would make `enabled_categories` and the distractor rules pick a side arbitrarily. **Schema 1 wrote a single `category` string**; it is migrated to a one-item list on load, before anything else touches the entry, and written back as a list.
